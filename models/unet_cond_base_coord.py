...
class Unet(nn.Module):
    ...
    def __init__(self, im_channels, model_config):
        ...
        if self.condition_config is not None:
            ...
            if 'text' in condition_types:
                ...
                self.text_embed_dim += self.coord_embed_dim # 좌표 정보 추가
    ...
    def forward(self, x, t, cond_input=None):
        ...
        if self.text_cond:
            ...
            ######### 좌표 conditioning ##########
            # 학습시 [ 16, 77, 512] = [batch_size, text_max_length, text embedding dim]
            coord_embed = torch.Tensor(cond_input['coords']).to(context_hidden_states.device).float()

            # sample 때는 input 하나이기 때문에 추가로 unsqueeze
            if len(coord_embed.shape) == 1:
                coord_embed = coord_embed.unsqueeze(0)
            coord_embed = coord_embed.unsqueeze(1)
            coord_embed = coord_embed.repeat(1, context_hidden_states.size(1), 1)
            context_hidden_states = torch.cat([context_hidden_states, coord_embed], dim=-1)
            ##############################################
        down_outs = []
