...
def sample(model, text_prompt, scheduler, train_config, diffusion_model_config,
           autoencoder_model_config, diffusion_config, dataset_config, vae, text_tokenizer, text_model, path_img):
    r"""
...
# def infer(args):
    ...
    jitter_coord = sample_config['jitter_coord']
    ...
    ########## sampling loop ###########
        # 각 불량별로 좌표 랜덤으로 뽑기
        df_coord = pd.read_csv(path_csv_fname_offset_class)
