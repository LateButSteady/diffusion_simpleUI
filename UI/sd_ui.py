...
def resource_path(relative_path):
    ...
    try:
        # PyInstaller에서 실행되는 경우 임시 경로에 위치
...
# config
path_icon       = r"G:\project\genAI\stable_diffusion_from_scratch\StableDiffusion-PyTorch\UI\icon.png"
path_ui         = r"G:\project\genAI\stable_diffusion_from_scratch\StableDiffusion-PyTorch\UI\sd_ui.ui"
path_help_md    = r"G:\project\genAI\stable_diffusion_from_scratch\StableDiffusion-PyTorch\UI\sd_ui_help.md"
path_config     = r"G:\project\genAI\stable_diffusion_from_scratch\StableDiffusion-PyTorch\config\Asdf_text_cond_coord_ui.yaml"
...
class WindowClass(QMainWindow, form_class):
    ...
    def initUI(self):
        ...
        # 임시 초기값 설정
        self.edit_pathImg.setText(r"G:\project\genAI\stable_diffusion_from_scratch\StableDiffusion-PyTorch\data\test_short")
        self.edit_taskName.setText("aaa")
        self.edit_epochVae.setText("1")
        self.edit_epochDdpm.setText("1")
        self.edit_timeStep.setText("500")
        self.edit_timeStep.setText("500")
    ...
    def loadConfig(self):
        try:
            with open(path_config, 'r') as file:
                ...
                self.append_to_console("config 파일 로딩 - OK")
 ...
 - {self.config["train_params"]["task_name"]}/vqvae_discriminator_ckpt.pth""")
    ...
    def check_emptyEditText(self, text: str, msg_dialog: str, msg_console: str) -> bool:
        ...
        if not text:
            # 다이얼로그 팝업
            self.show_error_dialog(msg_dialog)
            # 콘솔 출력
    ...
    def click_checkImgPath(self):
        ...
        # 새 스레드로 작업 실행
        self.thread_check_dataset = threading.Thread(target=self.check_img_path_in_thread)
        self.thread_check_dataset.start()


    def check_img_path_in_thread(self):
        """
        이미지 thread로 체크
        """
        try:
            ...
            self.show_error_dialog(msg=f"에러 발생: {e}")
        # finally:
    ...
    def prep_dataset_config(self) -> bool:
        ...
        ##### 데이터 경로 유효성 확인 #####
        if not self.checkImgPath():
            self.show_error_dialog(msg="이미지 폴더 경로를 확인하세요")
            self.append_to_console("[에러] 이미지 폴더 경로 유효하지 않음")
            return False
    ...
    def click_help(self):
        try:
            ...
            self.show_error_dialog(msg=f"도움말 불러오기 실패: {e}")
